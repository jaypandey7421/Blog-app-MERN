...
export const test = (req, res) =>{
    res.send(`API, route, and controller working`);
}
...
export const updateUser = async (req, res, next)=>{
    ...
    try{
        const updateUserOnReq = await User.findByIdAndUpdate(
            req.params.userId,
            {
                $set:{
        ...
        const {password, ...rest} = updateUserOnReq._doc;
...
export const deleteUser = async (req, res, next) =>{
    if(req.user.id !== req.params.userId){
        return next(errorHandler(403, "Oops! This operation can't be performed."));
    }
    try{
...
export const getUsers = async (req, res, next)=>{
    if(!req.user.isAdmin){
        return next(errorHandler(403, 'You are not allowed to view.'));
    }
